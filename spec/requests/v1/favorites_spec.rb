...
RSpec.describe 'V1::Favorites', type: :request do
  ...
  describe 'get /v1/hotels/:hotel_id/favorites - v1/favorites#index' do
    let_it_be(:client_user)  { create(:user) }
    let_it_be(:auth_tokens)  { client_user.create_new_auth_token }
    let_it_be(:hotel_manager)  { create(:user) }
    let_it_be(:accepted_hotel) { create(:accepted_hotel, user_id: hotel_manager.id) }
    ...
    context 'ログインしている場合' do
      it 'お気に入り一覧を取得できること' do
        get v1_hotel_favorites_path(accepted_hotel.id), headers: auth_tokens
        expect(response.status).to eq(200)
        expect(symbolized_body(response)[:body]).to eq('s')
      end
    end
    ...
    #   before do
    #     get v1_hotel_path(accepted_hotel.id), headers: hotel_manager_auth_tokens
    #   end

    #   it '400が返ること' do
    #     get v1_hotel_favorites_path(accepted_hotel), headers: auth_tokens
    #     expect(response.status).to eq(400)
    #   end
    # end
    ...
    #   it '400が返ること' do
    #     get v1_hotel_favorites_path(hidden_hotel.id), headers: auth_tokens
    #     expect(response.status).to eq(400)
    #   end
    # end
    ...
    #   it '400が返ること' do
    #     expect { get v1_hotel_favorites_path(accepted_hotel.id), headers: other_user_auth_tokens }.not_to change(Favorite, :count)
    #     expect(response.status).to eq(400)
    #   end
    # end
